...
- **拖入即用**：
  ...
  - 拖入 .txt / .epub / .mobi 小说 → 阅读器（GBK/UTF-8 自动识别、智能章节识别、按书记忆进度续读）
- **小说阅读器（参考https://github.com/binbyu/Reader） v2**：翻页/滚动双模式（点击翻页区/滚轮/←→/自动翻页）、排版自定义（字号/行距/段距/字距/边距/缩进）、四主题（纸白/羊皮/护眼/暗色）+ 自定义字色、书签、全文搜索、进度百分比跳转、章节识别规则可自定义
- **在线书源**：书城搜书 → 一键加入书架；章节按需抓取本地缓存，支持目录刷新追更；书源规则为 JSON（CSS 选择器），可导入管理
...
Electron 43 · 原生 JS（无框架）· WebContentsView 多标签 · node:test 单元测试 · jszip（epub）/ cheerio（书源 HTML 解析）/ iconv-lite（GBK）
